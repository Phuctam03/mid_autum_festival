...
function drawSky(time) {
  ...
  sparkles.forEach((s) => {
    const a = s.a * (0.5 + 0.5 * Math.sin(time * 0.001 * s.speed + s.phase));
    ctx.fillStyle = s.gold ? `rgba(255,205,99,${a})` : `rgba(225,230,255,${a})`;
    ctx.beginPath();
    ctx.arc(
      s.x + pointer.x * (s.r * 2),
  ...
  [bursts, fireworks].forEach((collection, group) => {
    for (let i = collection.length - 1; i >= 0; i--) {
      ...
      ctx.beginPath();
      ctx.arc(p.x, p.y, group ? 1.25 : p.size, 0, Math.PI * 2);
      ctx.fill();
      ctx.globalAlpha = 1;
      if (p.life <= 0) collection.splice(i, 1);
    }
  });
...
function buildHeroLanterns() {
  ...
  for (let i = 0; i < FESTIVAL.lanternCount; i++) {
    ...
    const side = i % 2 ? 5 + Math.random() * 19 : 76 + Math.random() * 19;
    el.style.cssText = `left:${side}%;top:${8 + Math.random() * 70}%;scale:${0.45 + Math.random() * 0.75};--speed:${3 + Math.random() * 4}s;animation-delay:${-Math.random() * 5}s;opacity:${0.45 + Math.random() * 0.55}`;
    layer.append(el);
  }
}
...
function enterFestival() {
  ...
  gsap
    ...
    .to(
      heroMoon,
      { scale: 5.6, filter: "brightness(2.3)", duration: 1.35 },
      "-=.35",
    )
    .to(flash, { opacity: 1, duration: 0.32 }, "-=.42")
    .set(world, {
      visibility: "visible",
      opacity: 1,
    ...
    .add(() => initThreeWorld())
    .to(flash, { opacity: 0, duration: 1.25, ease: "power2.out" })
    .fromTo(
      ".world-header",
      { y: -25, opacity: 0 },
    ...
    .to("#caption-card", { opacity: 1, y: 0, duration: 0.7 }, "-=.55");
  audio
    .play()
    .then(updateAudioState)
    .catch(() => updateAudioState());
...
function updateAudioState() {
  ...
  soundButton.querySelector(".sound-state").textContent = playing
    ? "SOUND ON"
    : "SOUND OFF";
  soundButton.setAttribute(
    "aria-label",
    playing ? "Pause festival music" : "Play festival music",
  );
}
...
function createGlowTexture() {
  ...
  g.addColorStop(0, "rgba(255,248,196,1)");
  g.addColorStop(0.25, "rgba(255,203,92,.55)");
...
function makePhotoCard(item, index, texture) {
  ...
  group.userData = { index, item, baseScale: 1 };
  const frame = new THREE.Mesh(
    new THREE.BoxGeometry(3.15, 2.35, 0.12),
  ...
  const image = new THREE.Mesh(
    new THREE.PlaneGeometry(2.91, 2.11),
...
function makeLantern3D(i) {
  ...
  const color = i % 3 === 0 ? 0xff9d42 : i % 3 === 1 ? 0xdc3435 : 0xf3c75b;
  ...
  const top = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xd7a546, metalness: 0.7 }),
  );
  top.position.y = 0.32;
  const tail = new THREE.Mesh(
    new THREE.CylinderGeometry(0.012, 0.012, 0.45, 5),
    new THREE.MeshBasicMaterial({ color: 0xffcc61 }),
  );
  ...
  g.position.set(
    Math.cos(a) * r,
    ((i % 5) - 2) * 1.25 + Math.sin(i) * 0.3,
    Math.sin(a) * r - 1.5,
  );
  g.userData = { phase: i * 0.71, speed: 0.5 + (i % 4) * 0.11 };
  return g;
}
...
function initThreeWorld() {
  ...
  scene.background = new THREE.Color(FESTIVAL.colors.night);
  scene.fog = new THREE.FogExp2(FESTIVAL.colors.night, 0.035);
  ...
  camera.position.set(0, mobile ? 0.3 : 0.8, mobile ? 14.5 : 15.5);
  ...
  scene.add(new THREE.AmbientLight(0x383667, 1.35));
  const moonLight = new THREE.PointLight(FESTIVAL.colors.moon, 65, 38, 1.8);
  ...
  scene.add(halo);
  worldMoon = new THREE.Mesh(
    new THREE.SphereGeometry(FESTIVAL.moonSize, 48, 48),
    new THREE.MeshStandardMaterial({
      color: 0xffe7a6,
      emissive: 0xf2ad55,
  ...
  worldMoon.position.set(0, 0.5, -2);
  worldMoon.userData.moon = true;
  scene.add(worldMoon);
  const starsGeo = new THREE.BufferGeometry();
  const starCount = mobile ? 350 : FESTIVAL.particleCount;
  const pos = new Float32Array(starCount * 3);
  for (let i = 0; i < starCount; i++) {
    const r = 14 + Math.random() * 22,
      a = Math.random() * Math.PI * 2,
      y = (Math.random() - 0.5) * 20;
    pos.set([Math.cos(a) * r, y, Math.sin(a) * r - 8], i * 3);
  }
  ...
  FESTIVAL.photos.forEach((item, index) => {
    loader.load(
      item.src,
      (texture) => {
        ...
        const card = makePhotoCard(item, index, texture);
        const angle = (index / FESTIVAL.photos.length) * Math.PI * 2;
        card.position.set(
          Math.cos(angle) * FESTIVAL.galleryRadius,
          Math.sin(angle * 2) * 1.7,
          Math.sin(angle) * FESTIVAL.galleryRadius - 1,
        );
  ...
  for (let i = 0; i < (mobile ? 10 : 18); i++) lanterns.add(makeLantern3D(i));
  scene.add(lanterns);
  ...
  for (let i = 0; i < 4; i++) {
    ...
    s.position.set(Math.cos(a) * 9, (i - 1.5) * 1.4, Math.sin(a) * 9 - 3);
    s.scale.set(1.2, 1.2, 1);
    scene.add(s);
  }
  raycaster = new THREE.Raycaster();
  clock = new THREE.Clock();
  renderer.domElement.addEventListener("pointermove", onWorldPointer);
  renderer.domElement.addEventListener("pointerdown", (e) => {
    dragStart = { x: e.clientX, rot: manualRotation };
  });
  renderer.domElement.addEventListener("pointerup", onWorldClick);
  renderer.domElement.addEventListener("pointerleave", () => {
    dragStart = null;
    hoverCard(null);
  });
  animateWorld(lanterns, halo);
}
...
function raycastCard(event) {
  ...
  return raycaster.intersectObjects(
    cards.flatMap((c) => (c ? [c.userData.image, worldMoon] : [])),
    false,
  )[0];
}
function hoverCard(card) {
  ...
  if (card) {
    ...
    const { index, item } = card.userData;
    $("#caption-card>span").textContent = String(index + 1).padStart(2, "0");
    $("#caption-card strong").textContent = item.caption;
    $("#caption-card small").textContent = item.detail.toUpperCase();
  }
}
function onWorldPointer(event) {
  ...
  if (dragStart) {
    manualRotation = dragStart.rot + (event.clientX - dragStart.x) * 0.004;
    return;
  }
  const hit = raycastCard(event);
  hoverCard(
    hit && !hit.object.userData.moon ? cards[hit.object.userData.index] : null,
  );
}
function onWorldClick(event) {
  const wasDrag = dragStart && Math.abs(event.clientX - dragStart.x) > 8;
  dragStart = null;
  if (wasDrag) return;
  const hit = raycastCard(event);
  if (!hit) return;
  if (hit.object.userData.moon) {
    worldMoonMagic(event);
    return;
  }
  openViewer(hit.object.userData.index);
}
...
function animateWorld(lanterns, halo) {
  const raw = clock.getDelta(),
    dt = Math.min(raw, 0.05),
    t = clock.elapsedTime;
  const auto = reducedMotion ? 0 : FESTIVAL.orbitSpeed;
  galleryGroup.rotation.y += auto * dt;
  galleryGroup.rotation.y +=
    (manualRotation - galleryGroup.rotation.y) * Math.min(1, dt * 2.2);
  galleryGroup.rotation.x +=
    (pointer.y * 0.045 - galleryGroup.rotation.x) * Math.min(1, dt * 2);
  camera.position.x +=
    (pointer.x * (mobile ? 0.35 : 0.85) - camera.position.x) *
    Math.min(1, dt * 1.8);
  camera.position.y +=
    ((mobile ? 0.3 : 0.8) + pointer.y * 0.45 - camera.position.y) *
    Math.min(1, dt * 1.8);
  camera.lookAt(0, 0.3, -1.4);
  cards.forEach((card, i) => {
    if (!card) return;
    const target = card.userData.hovered ? 1.18 : 1;
    card.scale.lerp(
      new THREE.Vector3(target, target, target),
      1 - Math.exp(-6 * dt),
    );
    card.position.y += Math.sin(t * 0.75 + i) * 0.0025;
    card.userData.glow.material.opacity +=
      (card.userData.hovered
        ? 0.28
        : 0.07 - card.userData.glow.material.opacity) * Math.min(1, dt * 5);
    card.position.z +=
      ((card.userData.hovered ? 1.05 : 0) -
        card.userData.image.position.z * 0.0) *
      0;
  });
  lanterns.children.forEach((l, i) => {
    l.position.y += Math.sin(t * l.userData.speed + l.userData.phase) * 0.002;
    l.rotation.z = Math.sin(t * 0.8 + i) * 0.08;
  });
  halo.material.opacity = 0.78 + Math.sin(t * 0.7) * 0.12;
  worldMoon.rotation.y += dt * 0.025;
  renderer.render(scene, camera);
  requestAnimationFrame(() => animateWorld(lanterns, halo));
}
...
function renderViewer() {
  const item = FESTIVAL.photos[viewerIndex];
  $("#viewer-image").src = item.src;
  $("#viewer-image").alt = item.caption;
  $("#viewer-index").textContent = String(viewerIndex + 1).padStart(2, "0");
...
addEventListener("pointermove", (e) => {
  pointer.targetX = (e.clientX / innerWidth) * 2 - 1;
...
heroMoon.addEventListener("click", moonMagic);
